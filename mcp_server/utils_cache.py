        ...
        try:
            with open(LOCAL_CACHE_FILE, "rb") as f:
                ...
                if isinstance(cached_data, dict) and "last_updated_timestamp" in cached_data:
                    ...
                    logger.error(f"Warning: Invalid cache file format found in {LOCAL_CACHE_FILE}.")
        except (pickle.UnpicklingError, EOFError, FileNotFoundError) as e:
            logger.error(f"Warning: Could not load cache from {LOCAL_CACHE_FILE}. Error: {e}")
        except Exception as e:
            logger.error(f"Warning: An unexpected error occurred while loading cache: {e}")
    return None
...
def save_cache_to_file(cache_data):
    ...
    try:
        ...
        logger.error(f"Warning: Failed to save cache to {LOCAL_CACHE_FILE}. Error: {e}")
...
async def init_cache(fetch_all_pairs_async):
    ...
    if loaded_cache:
        ...
        if (current_time - cache_timestamp) < CACHE_EXPIRY_SECONDS:
            ...
            logger.info(f"Local cache file {LOCAL_CACHE_FILE} is expired. Fetching fresh data.")
    ...
    try:
        ...
        for p in pairs_with_embeddings:
            ...
            try:
                ...
                logger.error(f"Warning: Could not decode embedding for pair {p.get('id', 'N/A')}. Error: {decode_err}")
        ...
        logger.error(f"ERROR: Failed to fetch data from backend during cache initialization: {fetch_err}")
        ...
        if loaded_cache:
            logger.info("Warning: Using potentially stale cache due to backend fetch failure.")
            PAIRS_CACHE.update(loaded_cache)
        else:
            logger.info("Warning: Proceeding without cache due to backend fetch failure and no valid local cache.")
...
async def update_cache(fetch_all_pairs_async):
    ...
    global PAIRS_CACHE
    logger.info("Force updating cache from backend...")
    # Temporarily remove local cache to force fetch in init_cache
    if os.path.exists(LOCAL_CACHE_FILE):
        try:
            os.remove(LOCAL_CACHE_FILE)
            logger.info(f"Temporarily removed {LOCAL_CACHE_FILE} to force refresh.")
        except Exception as e:
            logger.error(f"Warning: Could not remove local cache file for refresh: {e}")

    await init_cache(fetch_all_pairs_async)
    logger.info(f"Cache force updated at {time.ctime(PAIRS_CACHE.get('last_updated_timestamp', 0))}")
    return PAIRS_CACHE
